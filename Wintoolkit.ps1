...
#>
param(
    [ValidateSet("InfoSys","DiagBoot","AuditSOC","EDR","WinDiag","SFC","NetShare","ComparePC","EVCDiag","")]
    [string]$Module = "",
...
function Invoke-DiagBoot {
    ...
    if (-not $efiParts) {
        ...
        foreach ($efi in $efiParts) {
            ...
            $freeLetter  = ([char[]](67..90) | Where-Object { $usedLetters -notcontains $_ } | Select-Object -First 1)
            if ($freeLetter -and $efi.Guid) {
                $volId = "\\?\Volume{$($efi.Guid)}\"
                mountvol "${freeLetter}:" $volId 2>$null | Out-Null
                Start-Sleep -Milliseconds 800
                if (Test-Path "${freeLetter}:\EFI") {
                    Write-OK "EFI montee en ${freeLetter}: — Arborescence :"
                    Get-ChildItem -Recurse -Depth 3 "${freeLetter}:\EFI" -EA SilentlyContinue | ForEach-Object {
                    ...
                    if ($bootmgfw) {
                        ...
                        Write-OK "bootmgfw.efi present — Version : $($bfi.FileVersion)"
                    } else { Write-WARN "bootmgfw.efi Microsoft absent de cette EFI !" }
                } else {
                    Write-WARN "Partition montee mais \EFI\ introuvable."
                }
                mountvol "${freeLetter}:" /d 2>$null | Out-Null
                Write-INFO "Partition EFI demontee."
            } elseif (-not $efi.Guid) {
    ...
    $null       = $disks | Where-Object { $_.BusType -ne 'NVMe' -and $_.BusType -ne 'USB' } | Select-Object -First 1
    $nvmeWin    = $windowsInstalls | Where-Object { -not $_.IsSystem } | Select-Object -First 1
    $nvmeLetter = if ($nvmeWin) { $nvmeWin.Lettre } else { "D:" }
    ...
    if ($efiParts) {
        foreach ($efi in $efiParts) {
            if ($efi.DiskNumber -ne $sysDiskNum) {
                Write-ERR "CRITIQUE : EFI Disque $($efi.DiskNumber) != Windows Disque $sysDiskNum"
                Write-ACT "=> Appliquer plan section 11"
                $alertes++
            }
        }
    }
...
function Invoke-SFC {
    ...
    $timestamp6   = Get-Date -Format 'yyyyMMdd_HHmmss'
    $FilteredLog  = "$env:USERPROFILE\Desktop\CBS_SFC_DISM_Report_$timestamp6.txt"
    $HtmlReport   = "$env:USERPROFILE\Desktop\CBS_SFC_DISM_Report_$timestamp6.html"
    ...
    Get-ChildItem -Path "$env:USERPROFILE\Desktop\CBS_SFC_DISM_Report_*" -File -ErrorAction SilentlyContinue |
    ...
    <title>SFC·DISM Report — $timestamp6</title>
    <style>
